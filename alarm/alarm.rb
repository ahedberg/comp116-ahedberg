...
stream = PacketFu::Capture.new(:start => true, :iface => 'lo', \
                               :promisc => true)
alert_num = 0
...
stream.stream.each do |raw|
  ...
  begin
    # Check for nmap scans - TCP packets
    flags = packet.tcp_flags

    if (!(flags.urg && flags.ack && flags.psh && flags.rst && flags.syn && \
          flags.fin))
      # Found a NULL scan
      alert_num += 1
  ...
  # TODO Check for other nmap scans
  ...
  # TODO Check for in-the-clear password leakage
  if (payload.include? "PASS")
    alert_num += 1
    write_alert(alert_num, "Password leakage", packet.ip_saddr, packet.proto()[-1])
  end

  # TODO Check for in-the-clear credit card number leakage
